...
function Notifications() {
    ...
    const markAsSeen = async () =>{
        try {
            ...
            const response = await axios.post('/api/user/mark-notifications-as-seen', {userId : user._id}, {
                headers: {
    ...
    const deleteAll = async () =>{
        try {
            dispatch(showLoading())
            const response = await axios.post('/api/user/delete-notifications', {userId : user._id}, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`
                }
            })
